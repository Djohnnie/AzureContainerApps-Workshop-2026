...
Open a terminal in the `AzureContainerApps.Exercise01` project folder and run:

```bash
docker build -t azurecontainerapps-exercise01 .
```

| Flag | Description |
|------|-------------|
| `-t azurecontainerapps-exercise01` | Tags the built image with a name so you can reference it by name instead of its hash |
...
```bash
docker run --rm azurecontainerapps-exercise01
```
...
Open **Docker Desktop → Images** to confirm the `azurecontainerapps-exercise01` image was built, or run:

```bash
docker images azurecontainerapps-exercise01
```
