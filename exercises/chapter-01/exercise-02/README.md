...
> The project should be named AzureContainerApps.Exercise02.
...
Open a terminal in the `AzureContainerApps.Exercise02` project folder and run:

```bash
docker build -t azure-container-apps-exercise-02:latest .
```

| Flag | Description |
|------|-------------|
| `-t azure-container-apps-exercise-02:latest` | Tags the built image with a name so you can reference it by name instead of its hash |
...
```bash
docker run --rm -p 8080:8080 azure-container-apps-exercise-02:latest
```
...
Open **Docker Desktop → Images** to confirm the `azure-container-apps-exercise-02` image was built, or run:

```bash
docker images azure-container-apps-exercise-02
```
