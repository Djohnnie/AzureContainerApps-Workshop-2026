...
public class SnakeGameEngine
{
    ...
    public bool Tick()
    {
        ...
        // Remove tail first (unless eating), so the tail cell is freed before collision check
        if (!ateFood)
        {
        ...
        // Check self-collision against current body (tail already removed if not eating)
...
public enum CellType { Empty, SnakeHead, SnakeBody, Food }
